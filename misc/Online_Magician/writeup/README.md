...
本题主要考察ECDSA签名延展性攻击，需要提供关于一个特定消息的两个等效签名才能解出题目。

按常规理解通常会认为对于一个特定消息签名，一个签名者只能签出一个有效签名，但其实可以通过该签名数据在曲线上计算另外一个等效的点，即另一个可以ecrecover出同一个signer的等效签名。

主要变换在于签名数据vrs中的v和s这两部分，如下代码片段所示。关于其数学原理可以阅读[这篇文章](https://learnblockchain.cn/article/8281)深入学习理解。
...
可以看到`isSolved=True`，回到入口点交题即可得到flag为`SUCTF{C0n9r4ts!Y0u're_An_0ut5taNd1ng_OnchA1n_Ma9ic1an.}`
