...
import (
	...
	util "github.com/sirupsen/logrus"
)
...
func uploadFile(v *nfs.Target, path string, dest string) {
	...
	if bytes.Compare(actualSum, expectedSum) != 0 {
		util.Fatalf("sums didn't match. actual=%x expected=%s", actualSum, expectedSum) //  Got=0%x expected=0%x", string(buf), testdata)
	}

	util.Infof("Sums match %x %x", actualSum, expectedSum)
}

func catFile(v *nfs.Target, path string) {
	wr, err := v.Open(path)
	if err != nil {
		util.Errorf("read fail: %s", err.Error())
		return
...
func downloadFile(v *nfs.Target, path string) {
	...
	util.Printf("Sums match %x %x", actualSum, expectedSum)
	wr.Close()
	f.Close()
}
...
func main() {
	...
	uid := uint32(uid64)
	if err != nil {
		util.Fatalf("UID needs to be an integer - %v", err)
	}
	...
	gid := uint32(gid64)
	if err != nil {
		util.Fatalf("GID needs to be an integer - %v", err)
	}
	...
	if err != nil {
		util.Fatalf("unable to dial MOUNT service: %v", err)
	}
	defer mount.Close()
	...
	if err != nil {
		util.Fatalf("unable to mount volume: %v", err)
	}
	defer v.Close()
	...
	if err = mount.Unmount(); err != nil {
		util.Fatalf("unable to umount target: %v", err)
	}
	mount.Close()
